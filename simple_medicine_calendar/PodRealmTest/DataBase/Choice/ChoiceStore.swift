...
final class ChoiceStore: ObservableObject {
    ...
    private var itemResults: Results<ChoiceDB>
    
    init(realm: Realm) {
        itemResults = realm.objects(ChoiceDB.self)
            .sorted(byKeyPath: "id") // idの数値で並び替え stringが数値...?
    }
    
    var items: [ChoiceTable] {
        itemResults.map(ChoiceTable.init)
    }
    
}


extension ChoiceStore {
    // データの追加
    func create(choiceDrug: DrugDB) {
