...
final class RimindTimeStore: ObservableObject {
    
    private var itemResults: Results<RimindTimeDB>
    
    init(realm: Realm) {
        itemResults = realm.objects(RimindTimeDB.self)
            .sorted(byKeyPath: "id") // idの数値で並び替え stringが数値...?
    }
    
    var items: [RimindTimeItem] {
        itemResults.map(RimindTimeItem.init)
    }
    
}


extension RimindTimeStore {
    // データの追加
...
//
    ...
    func update(id: String, kisyou: String) {
        ...
        do {
            let realm = try Realm()
            try realm.write {
                realm.create(RimindTimeDB.self,
                             value: ["id": id,
                                     "kisyou": kisyou],
                             update: .modified)
            }
